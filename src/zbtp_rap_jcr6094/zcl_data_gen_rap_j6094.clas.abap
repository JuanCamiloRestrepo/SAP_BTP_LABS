CLASS zcl_data_gen_rap_j6094 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_gen_rap_j6094 IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    DELETE FROM ztravel_j6094_a.


    INSERT ztravel_j6094_a FROM (
    ...
    DELETE FROM zbooking_j6094_a.

    INSERT zbooking_j6094_a FROM (

        SELECT
          FROM /dmo/booking
          JOIN ztravel_j6094_a ON /dmo/booking~travel_id = ztravel_j6094_a~travel_id
          JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
          FIELDS  "client,
                  uuid( ) AS booking_uuid,
                  ztravel_j6094_a~travel_uuid AS parent_uuid,
                  ...
                  ztravel_j6094_a~last_changed_at AS local_last_changed_at ).


    DELETE FROM zbksppl_j6094_a.

    out->write( 'Adding Booking Supplements data' ).

    INSERT zbksppl_j6094_a FROM (
       SELECT FROM /dmo/book_suppl AS supp
              JOIN ztravel_j6094_a  AS trvl ON trvl~travel_id = supp~travel_id
              JOIN zbooking_j6094_a AS book ON book~parent_uuid = trvl~travel_uuid
